...
**[📦 最新 Release v1.24](https://github.com/wicanr2/wizardry-1-cht/releases/latest)** —
**完整可通關**的繁中重製版（從新角開到 B10F 沃登納結局）：

| 平台 | 包名 | 內容 |
|------|------|------|
| 🐧 Linux x86_64 | `wizardry-cht-x86_64.AppImage` | 單一檔案，雙擊即跑 — 含 wizardry_cht ELF + SDL2/_ttf/_image/_mixer + 全 assets。Ubuntu 20.04+ |
| 🪟 Windows x64 | `wizardry-cht-windows-x64.zip` | 解壓即玩 — `wizardry_cht.exe` + 所有 SDL2 系列 DLL + assets/ + `wizardry_cht.bat` 啟動器。Windows 10/11 |

兩個 package 都含：61 張 PCE-CD 怪物立繪 + 4 種衍生主題（Mono/Outline/Sepia + PCE-CD master）+
10 層原版 1981 迷宮 + 51 法術 + F1 操作說明 + 11 個音效 + 100% 中文化 + 日文/英文 fallback +
v1.24 沃登納結局合成畫面。背景音樂（4 軌, ~17 MB CC-BY Kevin MacLeod）由 `tools/fetch_music.sh` 抓。
...
### 群組 B-2：F3 切換視覺主題（v1.12 起、v1.20 全面深化）

任何場景按 **F3** 即輪替主題。**v1.20 之後 F3 不只是換怪物 sprite — 整個世界跟著換色**：

| 元素 | 主題切換時的變化（`src/render/palette.cpp`） |
|------|-----|
| **怪物立繪** | `PCECD` 彩色 → `Mono` 純黑白 → `Outline` 線稿 → `Sepia` CGA 鵝黃 |
| **迷宮 3D 牆面** | `palette.wall` 隨主題切換（PCECD 砂岩金 / Mono 純白 / Sepia 麥黃 / PC98 青藍） |
| **門 / 拱柱** | `palette.door` 跟換（PCECD 深紅 / Sepia 琥珀橘 / PC98 洋紅） |
| **地板 / 天花板** | 上半（天）下半（地）兩段填色，theme-specific（PCECD 暗藍 / Sepia 深褐 / Mac 藍圖灰） |
| **自動地圖** | wall line / door line / camera dot / frame 全部走 `palette` 取色 |
| **標題背景圖** | `assets/themes/<dir>/title/background.png` theme-aware 找圖，PCE-CD 用合成 splash，其他 fallback 純文字 bar |
| **BGM** | `assets/themes/<dir>/bgm/<scene>.ogg` 各主題可獨立 — 切 PC-98 主題即播 PC-98 版 BGM |

7 個 theme 都在 `render::theme::Theme` enum：`PCECD / Mono / Outline / Sepia / PC98 / WSC / Mac`。
按一次 F3 跳下一個（缺資料夾的自動跳過），右下角會顯示「✦ 切換主題：PCE-CD（彩色原版）」toast，
此時 **sprite cache 立即清空 + BGM epoch 增量** 確保整個畫面同步換皮。

![themes montage](docs/themes_montage.png)
*61 隻怪物在 4 種衍生主題下的對照（Mono/Outline/Sepia 由 `tools/generate_themes.py` 從 PCE-CD master 自動程序生成 — CC-BY-SA 4.0）。*

> 額外的 **PC-98 / WonderSwan / Macintosh / PCE-CD 本地音樂** 主題為「本機 only」—
> 使用者用 `tools/extract_*.sh` 從自己合法持有的 ROM 抽取，存到
> `assets/themes/{pc98,wsc,mac,pcecd_local}/`，不會 commit 到 repo。
...
### 群組 I：施法系統（v1.22 法術槽 + v1.21 戰鬥）

戰鬥中 **[S] 法術** → 選法術 → 選目標。`combat.cpp::cast_spell()` 進場先過 **slot gate**：
沒槽就印「**法力已盡**」、不消狀態、不發效果。每張角色卡都帶兩條陣列：

```
mage_spell_slots[7]     priest_spell_slots[7]
```

升級 → `recompute_spell_slots()` 自動補（Mage 直接、Bishop 副系延遲 3 級、Samurai 副系也延遲 3）。
旅館投宿 → `restore_spell_slots()` 補滿。Cap 9。**這是原版 1981 規則 1:1 復刻**。

施法時可看到的效果分類（51 條全接，v0.6 完整、v1.22 加上扣槽）：
- **群傷**：HALITO 1d8 / MAHALITO 4d6 / TILTOWAIT 10d15 滅世
- **狀態**：KATINO 沉睡 / MANIFO 麻痺 / MORLIS 恐懼 + AC+2
- **位置 / 偵測**：DUMAPIC 顯示 `B%dF X:%d Y:%d 面向北`
  - `CALFO` 看前方一格的 SquareFeature（v1.22 真的能用了）
  - `LATUMAPIC` 在營地施 → 下次戰鬥開頭 **g.identified=true**，整場日誌都顯示真名
- **照明**：MILWA 20 步 / LOMILWA 9999 步 — 在 dark zone 才有差，無光只見 1 格牆
- **復活**：DI（亡→生，可能失敗變灰燼）+ KADORTO（灰→生，可能永失）

### 群組 J：F4 三語切換（v1.15 + v1.20 chrome 收束）

按 **F4** 輪替 `繁中 → English → 日本語`。**453 個 UI 條目 + v1.20 新加的 ~30 個 chrome 鍵全部走 `i18n::tr()`**，
包括 F3/F4 toast、標題提示、底部 status hint、戰鬥突襲訊息、法力不足訊息、反魔法區域訊息、結局文字。

```cpp
// src/i18n/strings_zh_tw.json
"combat_vampire_drains": {
  "en":    "  -> %s is drained one life-level! (now level %d)",
  "zh_TW": "  -> %s 被吸取一級生命力！（剩 %d 級）",
  "ja_JP": "  -> %s はレベルを1吸い取られた！（残り %d ）"
}
```

`tr()` 查表時走 fallback chain `requested lang → en → zh_TW → key`，不會出現原始 key 字串。

### 群組 K：v1.24 結局畫面 — 沃登納倒下

![ending](assets/themes/pcecd/ending/background.png)

當隊伍在 B10F 戰勝 `WERDNA` 怪物群（戰鬥結尾偵測 monster name `find("WERDNA") != npos`），
`screens.cpp` 路由到 **`Scene::Ending`**：

- **PCE-CD theme** → 預先合成的 1280×720 結局背景（`tools/compose_ending_bg.py`）
  - 上層：OVA 城堡空照 + 金色 golden-hour 調色 + 底部 fade
  - 中層：金色護符發光圓盤 + 中央 "W" 符文
  - 下層：倒下的沃登納 sprite（去飽和、indigo 染色、半透明）
  - 全幅：4 環暈影 vignette 讓文字 panel 浮出
- **其他 theme** → 黑底純文字
- 文字：標題 banner + 4 行 epilogue + "按任意鍵回到標題"，**全部走 tr()** 三語齊備
