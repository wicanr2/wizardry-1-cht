...
|------|------|------|
| Pit | 坑洞 | 全隊每人 -1d8 HP |
| Chute | 滑道 | **強制掉到下一層**（無法回頭！） |
| Spinner | 旋轉盤 | 隨機改變朝向，**地圖完全失準** |
| Teleporter | 傳送器 | 傳送到該層或他層隨機位置 |
| Rock | 落石 | 卡住一回合 |
| Fizzle | 靜咒區 | **暫時禁止施法** |
| Stuck Door | 卡死門 | 需要力量檢定才能撞開 |
...
**機制**：靠近雕像 → 按 `SEARCH` → 觸發戰鬥 → 出現 **Murphy's Ghost**。

| Murphy's Ghost 資料 | 數值 |
|---|---|
| HP | 20–110（1d100 級別） |
| AC | **-3**（**對 1 級小隊難以命中**） |
| 傷害 | **1d1+1 = 2 點** |
...
> **進入提示**：起始 `5N-5E` 附近。
> 怪物：**Zombie（喪屍）**、**Coyote（郊狼）**、**Highwayman**、開始出現 **Gas Cloud（毒氣雲）**。
...
> **進入提示**：起始 `0N-0E`。
> 怪物：**Bishop**、**Gas Dragon**、**Arch Mage（大魔導師）**、**High Ninja（高階忍者）**。
...
它就是 **B9F 的 Murphy** ——傷害低但**幾乎打不到**，可以靠運氣賺大量 XP。
...
**第 7 chamber = Werdna's Lair**。

#### 敵人

- **Werdna**（HP 60、AC -7、能施 TILTOWAIT / MAHAMAN / MAKANITO）
- **Vampire Lord** × 1
- **Vampire** × 4
