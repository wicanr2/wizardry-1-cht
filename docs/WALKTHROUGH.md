...
下表是「**現在該不該繼續往下？該不該回城？**」的決策速查。等級指**全隊平均**，不是最高那位。

| 樓層 | 推薦進入等級 | 危險升級 | 主要農場 | 脫困關鍵法術 / 道具 | 退路 |
|------|------------|-----|---------|------------------|-----|
| B1F | 1-3 | — | Murphy's Ghost（5N-13E）| MILWA / KATINO / DIOS | 樓梯上 |
| B2F | 2-4 | 旋轉陷阱讓地圖失準 | Creeping Coins 群 | DUMAPIC 隨步走 | 樓梯上 |
| B3F | 3-5 | 30% 牆是暗門、有 Encounter 強制戰 | — | MILWA / LOMILWA（揭暗門） | 樓梯上 |
| B4F | 5-6 | **Trebor 軍隊邊界** — Blue Ribbon 解謎 + 假電梯 | Blue Ribbon | LOMILWA + DIAL | 電梯（B9F 速通） |
| B5F | 6-7 | **Chute 強制下樓**、食人魔群 | Ogre Lord | DIALMA / MAPORFIC | Chute = 不可回頭 |
| B6F | 7-8 | 怪物開始 MAHALITO / DALTO，**寶箱陷阱** | Bishop 法師群 | MONTINO（靜默施法者）/ CALFO | LOKTOFEIT（Lvl6） |
| B7F | 8-10 | **Arch Mage 自帶 TILTOWAIT** | 大魔導師（同 Werdna XP）| MONTINO + ZILWAN | LOKTOFEIT |
| B8F | 9-11 | **Vampire / Vampire Lord 吸等級**，巨人群 | High Ninja | ZILWAN（不死秒殺）+ MAPORFIC | LOKTOFEIT |
| B9F | 10-12 | **Lifestealer 全遊戲最強吸等級**、Will-o-Wisp AC -8 | Will-o-Wisp XP 4 萬 2 | ZILWAN + MOGATO | Chute → B10F（速殺路） |
| B10F | 11-13 | **Contra Dextra 暗道、Werdna 群** | — | MONTINO + ZILWAN + TILTOWAIT + MADI | MALOR（瞬移）回城 |

> 「**推薦進入等級**」 = 你下去**不會被 30 秒內團滅**的最低門檻。
> 「**危險升級**」 = 跟上一層比，新增加的「會殺死整隊」的機制。
> 「**脫困關鍵法術**」 = 卡住時最該預備好的咒語/道具。
